...
<body>
  ...
  <script>
    ...
    const VALID_KEYS = [
    { value: "0x00", text: "" },
    { value: "0x1", text: "KEY_LEFT_CTRL" },
    { value: "0x2", text: "KEY_LEFT_SHIFT" },
    { value: "0x4", text: "KEY_LEFT_ALT" },
    { value: "0x8", text: "KEY_LEFT_GUI" },
...
function addRow(rowData = null) {
    ...
    newRow.innerHTML = `
        ...
        <td>${createBooleanDropdown(defaultValues[12])}</td>
...
// Funktion: Protokoll-Dropdown erstellen
...
// Funktion: Routendaten vom ESP laden
...
// Funktion: Alle Zeilen aktualisieren
function updateAllRows() {
    const rows = document.querySelectorAll("#dataTable tbody tr");
    rows.forEach(row => {
        toggleFields(row.querySelector("select"));
        toggleRouteFields(row.querySelector("select:nth-of-type(1)"));
    });
}
